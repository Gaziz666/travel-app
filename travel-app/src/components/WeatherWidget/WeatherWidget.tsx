...
const WeatherWidget: React.FC<Props> = ({ countriesLoaded, selectedCountryIndex, countries, selectedLanguage }) => {
    let countryCapitalEn = '';
    let countryCapital = '';

    if (countries.length > 0) {
        countryCapitalEn = countries[selectedCountryIndex].translations.en.capital;
        countryCapital = countries[selectedCountryIndex].translations[selectedLanguage].capital;
    }
    ...
    useEffect(() => {
        const WEATHER_API = `http://api.openweathermap.org/data/2.5/weather?q=${countryCapitalEn}&appid=630198738706e6feb41dd55034d68b96&units=imperial&lang=${selectedLanguage}`
        const request = async () => {
            try {
